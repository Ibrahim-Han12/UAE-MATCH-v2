...
import { matchesApi, coachApi } from '../lib/api';
...
export function DailyRecommendationsPage({ onLike, dailyLikesUsed, dailyLikesLimit, isVIP = false }: DailyRecommendationsPageProps) {
  ...
  const [selectedUserInsights, setSelectedUserInsights] = useState<any>(null);
  ...
  useEffect(() => {
    const loadRecommendations = async () => {
      try {
        setLoading(true);
        const data = await matchesApi.getSuggestions();
        setRecommendations(data);
      } catch (error: any) {
        console.error('加载推荐失败:', error);
        // 失败时使用模拟数据
        setRecommendations(DAILY_RECOMMENDATIONS);
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const loadInsights = async () => {
      if (selectedIndex !== null && recommendations[selectedIndex]) {
        const user = recommendations[selectedIndex];
        try {
          setLoadingInsights(true);
          const insights = await coachApi.getMatchInsights(parseInt(user.id));
          
          // 转换格式以匹配前端期望
          setSelectedUserInsights({
            matchScore: Math.round(insights.match_score * 100),
            matchReason: insights.analysis?.split('\n')[0] || '匹配度较高',
            aiAnalysis: {
              compatibility: insights.match_score_breakdown ? 
                Object.entries(insights.match_score_breakdown).map(([label, score]: [string, any]) => ({
                  label,
                  score: Math.round(score * 100),
                  reason: insights.analysis || '',
                })) : [],
          ...
          setSelectedUserInsights({
            ...
            matchReason: user.matchReason || '匹配度较高',
  ...
  const handlePass = async (userId: string) => {
    try {
      // 调用后端 API
      await matchesApi.performAction(parseInt(userId), 'pass');
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-sm p-8 text-center border border-gray-200">
          ...
          <p className="text-gray-600">正在加载推荐...</p>
        </div>
      </div>
    );
  }
            ...
            <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl p-6 border border-orange-100">
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    综合匹配度：{selectedUserInsights?.matchScore || currentUser!.matchScore || 85}%
                  </p>
                </div>
                {isVIP && (
              ...
              {loadingInsights && (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-600">正在生成分析报告...</p>
                </div>
              )}

              {/* 简化版分析（普通用户） */}
              {!loadingInsights && !isVIP && (
                <>
                  {/* 基础匹配原因 */}
                  <div className="bg-white rounded-lg p-4 mb-4">
                    <h4 className="text-sm mb-2">匹配原因</h4>
                    <p className="text-sm text-gray-700">
                      {selectedUserInsights?.matchReason || currentUser!.matchReason || '匹配度较高'}
                    </p>
                  </div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="space-y-3">
                      {(selectedUserInsights?.aiAnalysis?.compatibility || currentUser!.aiAnalysis?.compatibility || []).slice(0, 2).map((item: any, i: number) => (
              ...
              {!loadingInsights && isVIP && (
                <>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="space-y-3">
                      {(selectedUserInsights?.aiAnalysis?.compatibility || currentUser!.aiAnalysis?.compatibility || []).map((item: any, i: number) => (
                        <div key={i} className="bg-white rounded-lg p-4">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-sm">{item.label}</span>
                            <span className="text-sm text-[#E07A5F]">{item.score}%</span>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="space-y-2">
                      {(selectedUserInsights?.aiAnalysis?.highlights || currentUser!.aiAnalysis?.highlights || []).map((item: string, i: number) => (
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {(selectedUserInsights?.aiAnalysis?.considerations || currentUser!.aiAnalysis?.considerations || []).map((item: string, i: number) => (
                        <div key={i} className="flex items-start gap-2 bg-white rounded-lg p-3">
