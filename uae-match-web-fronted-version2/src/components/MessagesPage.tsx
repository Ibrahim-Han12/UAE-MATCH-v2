...
export function MessagesPage({ conversations, currentUserId, onSendMessage }: MessagesPageProps) {
  ...
  const [conversationsWithMessages, setConversationsWithMessages] = useState<Conversation[]>(conversations);
  ...
  useEffect(() => {
    const loadMessages = async () => {
      ...
      const conversation = conversationsWithMessages.find(c => c.id === selectedConversation);
      if (!conversation || !conversation.matchPairId) return;
      ...
      try {
        ...
        const messages = await chatApi.getMessages(conversation.matchPairId);
        
        // 转换格式
        const formattedMessages: Message[] = messages.map((msg: any) => ({
          id: msg.id?.toString() || Date.now().toString(),
          text: msg.content || '',
          sender: msg.is_me || msg.from_me ? currentUserId : 'other',
    ...
    loadMessages();
  }, [selectedConversation, conversationsWithMessages, currentUserId]);

  // 当 conversations 更新时，同步更新 conversationsWithMessages
  useEffect(() => {
  ...
  const currentConversation = conversationsWithMessages.find((c) => c.id === selectedConversation);
            ...
            {loadingMessages ? (
              <div className="flex items-center justify-center h-full">
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-500">加载消息...</p>
                </div>
              </div>
        ...
        {conversationsWithMessages.length === 0 ? (
          ...
          <div className="space-y-3">
            {conversationsWithMessages.map((conversation) => {
              ...
              const hasUnread = (conversation as any).unreadCount > 0;
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-500 truncate">
                      {lastMessage ? lastMessage.text : (conversation as any).lastMessage || '开始聊天吧'}
                    </p>
                  </div>
                  {hasUnread && (
