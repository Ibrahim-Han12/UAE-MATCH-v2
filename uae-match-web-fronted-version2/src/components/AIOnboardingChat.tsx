...
export function AIOnboardingChat({ onComplete }: AIOnboardingChatProps) {
  ...
  useEffect(() => {
    const initializeChat = async () => {
      try {
        ...
        // 尝试加载历史对话
        const history = await aiChatApi.getHistory('registration', 50);
        ...
        if (history && history.length > 0) {
          ...
          // 找到当前应该问的问题
          // 简化处理：如果对话已有很多，直接到最后一步
          if (formattedMessages.length > 10) {
            setCurrentQuestionIndex(AI_QUESTIONS.length - 2);
          }
        } else {
          // 没有历史对话，开始新对话
          // 调用后端开始注册对话
          try {
            const response = await aiChatApi.startRegistration();
            if (response.message) {
  ...
  const handleSendMessage = async () => {
    ...
    setIsAITyping(true);
    try {
      const response = await aiChatApi.sendMessage(userInput, 'registration');
      ...
      // 检查是否完成（AI 可能会主动完成）
      // 这里简化处理：如果 AI 回复包含"总结"或"提交"等关键词，认为完成
      if (response.content.includes('总结') || response.content.includes('提交') || response.content.includes('完成')) {
        // 完成收集，生成画像
        ...
        setTimeout(async () => {
          try {
            // 调用后端完成注册
            const response = await aiChatApi.completeRegistration(true);
            ...
            setTimeout(() => {
              ...
              onComplete({
                ...newUserData,
                profileCreated: response.profile_created || false,
              });
            }, 2000);
          } catch (error: any) {
