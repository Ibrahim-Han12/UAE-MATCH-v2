...
| AI | openai SDK；`gpt-4o-mini` / `text-embedding-3-small`(1536维) | ⚠️ 有 config 层但未形成"任务→档位"抽象层（CLAUDE §4） |
...
- 🔴 **L3 主动性引擎**：现有 `vip_care_service`（周二/五定时）为雏形；PRD 要求**全用户分级事件引擎 + 日历事件 + 伦理护栏同步**（BR-204/206，v1.0 范围）。
- 🔧 **模型路由与计量**：现有 config + fallback，但未形成抽象层；PRD/CLAUDE §4 要求"任务→档位"路由 + Prompt Caching + BR-209 计量。
- ♻️ ai_conversations / user_token_usage / global_ai_budget / user_embeddings 可复用。

### 4.4 匹配与推荐（PRD 6 / BR-301~307）
- 🔴 **交互模型**：现为滑动/每日推荐/发现页 + like/pass；PRD 要求**每周 1–3 封推荐信、周五 20:00 送达、先理由后照片、三动作、72h 时限**（BR-301/302/307）。
...
- 🔴 **商品目录**：现为人民币道具制（`payment.py:30` 起：¥29/¥59 + super_like/boost/who_liked_me）；PRD 要求 **Stripe AED 订阅四层 + 冻结道具（下线入口、接口返 410）**（BR-501/503）。
...
① 模型抽象层 + 计量（BR-209，其他 AI 工作的前提）→ ② 三层记忆（BR-202，地基）→ ③ 账户/状态机/EID 闸（BR-001/107）→ ④ 深访三层架构（BR-201）→ ⑤ 推荐信流水线（BR-301）→ ⑥ Stripe AED 订阅（BR-501/503）。
