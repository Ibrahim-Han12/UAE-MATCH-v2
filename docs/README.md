...
| [open-questions.md](open-questions.md) | 待决问题与裁决记录 |

## M2 开工顺序（hld-current-state §7）

```
①模型抽象层+计量(BR-209) → ②三层记忆(BR-202, 建在 Postgres)
→ ③账户/状态机/EID闸(BR-001/107) → ④深访三层架构(BR-201)
→ ⑤推荐信流水线(BR-301) → ⑥Stripe AED 订阅(BR-501/503)
```
...
BRD/PRD=业务/产品需求 ｜ HLD/LLD=概要/详细设计 ｜ PDPL=UAE 个人数据保护法 ｜ EID=阿联酋身份证 ｜ 小缘=AI 红娘 ｜ S0–S7=用户状态机（PRD 2.2） ｜ BR-XYY=业务需求编号
